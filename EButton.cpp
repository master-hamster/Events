	...
	if ( debouncingStarted ) {
		if ( debounceTimer.expired() ) {
			...
			if ( this->currentState != this->currentData ) {
				...
				if ( eventType != evNone) {
					DBG_PRINT( F("EButton::idle(): rise eventType=") );
					DBG_PRINT( eventType );
					DBG_PRINT( F(" Src:") );
					DBG_PRINTLN( getID() );
					riseEvent( eventType );
				...
				if ( ( holdTimer.expired() ) && ( this->currentState == 1 ) ) {
					DBG_PRINTLN( F("EButton::idle point 3") );
		...
		if ( this->currentState != this->currentData ) {
			...
			DBG_PRINT( F("EButton::idle(): start debouncing, newstate=") );
			DBG_PRINTLN( this->currentData );
